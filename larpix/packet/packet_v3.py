...
class Packet_v3(object):
    '''
    Representation of a 64 bit LArPix v3 UART data packet.

    Packet_v3 objects are internally represented as bitarrays, but a variety of
    helper properties allow one to access and set the data stored in the bitarrays in
    a natural fashion. E.g.::

        p = Packet_v3() # initialize a packet of zeros
    ...
    Packet_v3 objects don't enforce any value validation, so set these fields with caution!

    In FIFO diagnostics mode, the bits are to be interpreted in a different way.
    At this point, there is no way for the ``Packet_v3`` to automatically know if it
    is in FIFO diagnostics mode. If you are operating the chips in this mode you
    can manually set ``Packet_v3.fifo_diagnostics_enabled = True`` to interpret
    ...
    timestamp_bits = slice(16,44)
    reset_sample_flag_bits = slice(44, 45)
    cds_flag_bits = slice(45, 46)
    dataword_bits = slice(46,56)
    ...
    def __repr__(self):
        return 'Packet_v3(' + str(self.bytes()) + ')'
    ...
    def export(self):
        ...
        if ptype == self.TEST_PACKET:
            ...
            d['cds_flag'] = self.cds_flag
            d['reset_sample_flag'] = self.reset_sample_flag
            if self.fifo_diagnostics_enabled:
    ...
    def from_dict(self, d):
        '''
        Load from a dict of values, inverse of ``Packet_v3.export``
        ...
        if 'type' in d and d['type'] not in (self.DATA_PACKET, self.TEST_PACKET, self.CONFIG_WRITE_PACKET, self.CONFIG_READ_PACKET):
            raise ValueError('invalid packet type for Packet_v3')
    ...
    @property
    def cds_flag(self):
        bit_slice = self.cds_flag_bits
        return bah.touint(self.bits[bit_slice], endian=self.endian)

    @property
    def reset_sample_flag(self):
        bit_slice = self.reset_sample_flag_bits
        return bah.touint(self.bits[bit_slice], endian=self.endian)
...
Packet_v3.packet_type = property(Packet_v3._basic_getter('packet_type'),Packet_v3._basic_setter('packet_type'))
Packet_v3.downstream_marker = property(Packet_v3._basic_getter('downstream_marker'),Packet_v3._basic_setter('downstream_marker'))
Packet_v3.parity = property(Packet_v3._basic_getter('parity'),Packet_v3._basic_setter('parity'))
Packet_v3.channel_id = property(Packet_v3._basic_getter('channel_id'),Packet_v3._basic_setter('channel_id'))
Packet_v3.dataword = property(Packet_v3._basic_getter('dataword'),Packet_v3._basic_setter('dataword'))
Packet_v3.trigger_type = property(Packet_v3._basic_getter('trigger_type'),Packet_v3._basic_setter('trigger_type'))
Packet_v3.register_address = property(Packet_v3._basic_getter('register_address'),Packet_v3._basic_setter('register_address'))
Packet_v3.register_data = property(Packet_v3._basic_getter('register_data'),Packet_v3._basic_setter('register_data'))
Packet_v3.local_fifo = property(Packet_v3._basic_getter('local_fifo'),Packet_v3._basic_setter('local_fifo'))
Packet_v3.shared_fifo = property(Packet_v3._basic_getter('shared_fifo'),Packet_v3._basic_setter('shared_fifo'))
Packet_v3.magic_word = property(Packet_v3._basic_getter('magic_word'),Packet_v3._basic_setter('magic_word'))
